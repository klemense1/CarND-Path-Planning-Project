...
double TrajectoryPlanner::evalCoefficients(vector<double> poly, double t) {
    return poly[0] + poly[1] * t + poly[2] * pow(t,2) + poly[3] * pow(t,3) + poly[4] * pow(t,4) + poly[5] * pow(t,5);
  }

vector<double> TrajectoryPlanner::JMT(const vector< double> start, const vector<double> end, double T)
  {
    /*
...
const vector<vector<double>> TrajectoryPlanner::getLastSentTrajectory() {
